...
import {
    ...
    Paper, TextField, Stack, Chip, Divider, CircularProgress
} from '@mui/material';
import SearchIcon from '@mui/icons-material/Search';
import ConflictAlert from './ConflictAlert.jsx';
...
const CourseList = () => {
    ...
    useEffect(() => {
        if (!user || !user.year) {
            setLoading(false);
            return; // Don't fetch if user or year is not defined
        }
        
        const fetchCourses = async () => {
            try {
                setLoading(true);
                // Fetch courses for the student's specific year
                const res = await axios.get(`${API_URL}/api/courses?year=${user.year}`);
                setCourses(res.data);
            } catch (error) {
    ...
    const handleRegister = async (courseId) => {
        try {
            ...
            window.location.reload(); // Reload to update schedule
        } catch (error) {
            if (error.response && error.response.status === 409) {
                setConflict({ message: error.response.data });
            } else {
    ...
    return (
        <Box>
            <ConflictAlert conflict={conflict} onDismiss={() => setConflict(null)} />
            ...
            <Paper elevation={2} sx={{ p: 2, mb: 4, borderRadius: '12px' }}>
                <Stack direction="row" spacing={2} alignItems="center">
                    ...
                    <TextField
                        fullWidth
                        variant="standard"
                        placeholder={`Search ${user.year}st year courses, instructors, or course codes...`}
                        ...
                        <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', borderRadius: '12px', boxShadow: 3 }}>
                            <CardContent sx={{ flexGrow: 1 }}>
                                <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
                                    <Typography variant="h6" component="div">{course.courseCode}: {course.title}</Typography>
                                ...
                                <Typography sx={{ mt: 1, fontStyle: 'italic' }} color="text.secondary">Prof. {course.instructor}</Typography>
                                <Typography sx={{ mt: 1 }} variant="body2">{course.credits} Credits</Typography>
                                <Divider sx={{ my: 2 }} />
                                {course.schedule.map((slot, index) => (
                                    <Stack key={index} direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 1 }}>
                                        <Typography variant="body2">{slot.day}: {slot.startTime} - {slot.endTime}</Typography>
